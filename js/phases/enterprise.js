...
<span class="cmt"># Then update remote URLs to use new key</span></span>`}}
  ...
  challenges:[
    {q:"Type the git config command to make Git automatically sign ALL commits using GPG by default.",scenario:"Your company's security policy requires all commits to be signed. You want to enable this globally.",hint:"It's a git config setting: commit.gpgsign set to true.",answer:"Answer: git config --global commit.gpgsign true — This makes Git automatically sign every commit with your configured GPG key. You also need to set user.signingkey to your GPG key ID.",accept:["git config --global commit.gpgsign true","git config --global commit.gpgsign true ","git config --global commit.gpgsign 1"],feedback:"commit.gpgsign true makes Git sign every commit automatically with your configured signing key. Without this, you'd need to add -S to every commit command. Set this globally so it applies to all your repos."},
    {q:"A secret API key was committed and pushed 3 commits ago. What is the FIRST thing you must do, before even cleaning the Git history?",scenario:"You just discovered AWS_SECRET_KEY was committed to a public repo. What's the immediate priority?",hint:"Before ANY Git cleanup: immediately invalidate the exposed credential so it can't be used.",answer:"Answer: Revoke/rotate the secret immediately. Before any Git cleanup (BFG, filter-repo), the exposed credential must be invalidated because it\'s already in the remote\'s history and may have been pulled by others. Cleaning Git history alone doesn\'t protect you.",accept:["revoke the secret","revoke or rotate the secret","invalidate the secret","rotate the key","revoke the key","revoke and rotate the exposed secret","revoke the exposed credential"],keywords:["revoke|rotate|invalidate|change|disable|expire|cancel|regenerate"],feedback:"ALWAYS revoke/rotate the exposed secret first — immediately. Git history cleaning takes time and even if you rewrite history, the secret was already exposed during the window it was public. Treat it as compromised the moment it was pushed. Clean the history after the secret is already invalidated."},
    {q:"Type the git log flag that shows the GPG/SSH signature verification status alongside each commit.",scenario:"You want to audit recent commits to confirm they all have verified signatures.",hint:"There's a --show-signature flag for git log that displays verification status.",answer:"Answer: --show-signature — Use with git log: git log --show-signature. Each commit shows \'Good signature from...\' or \'No signature\'. This verifies commit authenticity and integrity.",accept:["--show-signature","--show-signature "],feedback:"git log --show-signature shows the signature status for each commit — 'Good signature from...' for valid signatures, or an error for invalid/missing ones. Use this for security audits to ensure all commits on a branch are properly signed."}
  ]
},
{
...
<span class="cmt"># Stored in .git/rr-cache/</span></span>`}}
  ...
  challenges:[
    {q:"Type the git log flag that tracks a file's history even across renames.",scenario:"A file was renamed from 'utils.js' to 'helpers.js' 20 commits ago. You want to see its complete history including before the rename.",hint:"A single flag that tells git log to trace the file through rename operations.",answer:"Answer: --follow — Use with git log: git log --follow filename. Without this flag, git log stops at the rename event. With it, Git traces the file through rename operations to show its full history.",accept:["--follow","--follow "],feedback:"git log --follow -- filename traces a file's history through renames. Without --follow, git log stops at the rename and you lose all older history. Always use --follow when investigating a file that may have been renamed."},
    {q:"Type the git log flag and pattern to find every commit where the string 'DROP TABLE' was either added or removed from any file.",scenario:"Security audit: you need to find every commit in history that touched any SQL DROP TABLE statement.",hint:"The -S flag (called the 'pickaxe') searches for when a string was added or removed in diffs.",answer:"Answer: git log -S \"DROP TABLE\" — The -S flag (called the \'pickaxe\') searches for commits where the given string was added or removed. It\'s not a grep — it finds the commits that changed the count of that string.",accept:["git log -s \"drop table\"","-s \"DROP TABLE\"","-S \"DROP TABLE\"","-S 'DROP TABLE'","git log -s 'drop table'","git log -S \"DROP TABLE\"","git log -S 'DROP TABLE'","--pickaxe -s drop table"],feedback:"The -S flag (the 'pickaxe') searches for commits where the string was added or removed in the diff content. It's different from --grep which only searches commit messages. Use -G for regex patterns instead of literal strings."},
    {q:"What git config setting enables automatic reuse of previously recorded conflict resolutions?",scenario:"Your team frequently rebases long-running branches onto main and resolves the same conflicts repeatedly. What setting eliminates this repetition?",hint:"The feature is called 'rerere' — Reuse Recorded Resolution. It's a config setting.",answer:"Answer: rerere.enabled true — Set with: git config --global rerere.enabled true. \'rerere\' stands for \'Reuse Recorded Resolution\'. Git remembers how you resolved a conflict and automatically applies the same resolution if it sees the same conflict again.",accept:["rerere.enabled true","rerere.enabled = true","git config rerere.enabled true","git config --global rerere.enabled true","rerere.enabled","rerere","git rerere","rerere enabled true","rerere.enabled=true"],feedback:"rerere (Reuse Recorded Resolution) records the before/after state of every conflict you resolve. Next time Git encounters the identical conflict, it applies your resolution automatically. Huge time saver on teams that do frequent rebases."}
  ]
},
{
...
<span class="cmt"># GitLab: Settings → Repository → Push rules → Reject unsigned commits</span></span>`}},
  ...
  challenges:[
    {q:"What file do you create to automatically request reviews from specific people or teams when certain files change in a PR?",scenario:"Your org requires the database team to review every migration file. How do you enforce this automatically?",hint:"The file name literally describes who owns the code. ALL CAPS, one word.",answer:"Answer: CODEOWNERS file. Place it in the repo root or .github/ directory. Each line maps a file pattern to reviewers — e.g. /src/api/ @backend-team. GitHub auto-assigns reviewers when PRs touch matching paths.",accept:["codeowners","CODEOWNERS",".github/codeowners",".github/CODEOWNERS","the CODEOWNERS file","a CODEOWNERS file"],feedback:"CODEOWNERS maps file patterns to GitHub users (@username) or teams (@org/team-name). When a PR modifies matching files, those owners are automatically added as required reviewers. Works with branch protection's 'Require review from Code Owners' setting to enforce it."},
    {q:"Name the THREE recommended merge strategies available on GitHub for merging pull requests.",scenario:"Your engineering manager asks you to explain the options and recommend one for a team practicing trunk-based development.",hint:"Think: full merge commit, squash everything into one, or replay commits linearly.",answer:"Answer: Merge commit, squash and merge, rebase and merge. Merge commit preserves full branch history. Squash condenses all branch commits into one. Rebase replays commits linearly onto the target branch. Each has trade-offs for history readability.",accept:["merge commit, squash and merge, rebase and merge","merge commit squash and merge rebase and merge","squash and merge, merge commit, rebase and merge","merge, squash, rebase","squash merge rebase","create a merge commit, squash and merge, rebase and merge","merge squash rebase","merge commit, squash, rebase","squash and merge, rebase and merge, merge commit","rebase and merge, squash and merge, merge commit","merge, squash and merge, rebase and merge","regular merge, squash merge, rebase merge"],keywords:["merge","squash","rebase"],feedback:"GitHub offers: Merge commit (creates a merge commit, preserves all PR commits), Squash and merge (combines all PR commits into one on main — cleanest for trunk-based dev), Rebase and merge (replays PR commits linearly without a merge commit). Most enterprise teams enforce squash-only for a clean, readable main history."},
    {q:"Type the git log format string to output commits with their full hash, ISO 8601 timestamp, author email, and subject — suitable for an audit log export.",scenario:"Your security team needs a machine-readable audit trail of all commits across all branches.",hint:"Use --format with placeholders: %H (hash), %aI (ISO date), %ae (email), %s (subject). Add --all for every branch.",answer:"Answer: git log --format=\'%H|%aI|%ae|%s\' --all — Placeholders: %H = full hash, %aI = ISO date, %ae = author email, %s = subject. The --all flag includes every branch. Pipe output to scripts for reporting.",accept:["git log --format=\"%h %ai %ae %s\" --all","git log --format=\"%H %aI %ae %s\" --all","git log --format='%H %aI %ae %s' --all","git log --format='%h %aI %ae %s' --all","--format=\"%h %ai %ae %s\" --all","--format=\"%H %aI %ae %s\" --all"],feedback:"%H = full hash, %aI = ISO 8601 strict author date (better for parsing than %ai), %ae = author email, %s = subject line. The --all flag includes every branch and tag. Pipe this to a file or your SIEM system for compliance logging."}
  ]
}
);