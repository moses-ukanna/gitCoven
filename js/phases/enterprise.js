...
<span class="cmt"># Then update remote URLs to use new key</span></span>`}}
  ...
  challenges:[
    ...
    {q:"A secret API key was committed and pushed 3 commits ago. What is the FIRST thing you must do, before even cleaning the Git history?",scenario:"You just discovered AWS_SECRET_KEY was committed to a public repo. What's the immediate priority?",hint:"Before ANY Git cleanup: immediately invalidate the exposed credential so it can't be used.",accept:["revoke the secret","revoke or rotate the secret","invalidate the secret","rotate the key","revoke the key","revoke and rotate the exposed secret","revoke the exposed credential"],keywords:["revoke|rotate|invalidate|change|disable|expire|cancel|regenerate"],feedback:"ALWAYS revoke/rotate the exposed secret first — immediately. Git history cleaning takes time and even if you rewrite history, the secret was already exposed during the window it was public. Treat it as compromised the moment it was pushed. Clean the history after the secret is already invalidated."},
...
<span class="cmt"># GitLab: Settings → Repository → Push rules → Reject unsigned commits</span></span>`}},
  ...
  challenges:[
    ...
    {q:"Name the THREE recommended merge strategies available on GitHub for merging pull requests.",scenario:"Your engineering manager asks you to explain the options and recommend one for a team practicing trunk-based development.",hint:"Think: full merge commit, squash everything into one, or replay commits linearly.",accept:["merge commit, squash and merge, rebase and merge","merge commit squash and merge rebase and merge","squash and merge, merge commit, rebase and merge","merge, squash, rebase","squash merge rebase","create a merge commit, squash and merge, rebase and merge","merge squash rebase","merge commit, squash, rebase","squash and merge, rebase and merge, merge commit","rebase and merge, squash and merge, merge commit","merge, squash and merge, rebase and merge","regular merge, squash merge, rebase merge"],keywords:["merge","squash","rebase"],feedback:"GitHub offers: Merge commit (creates a merge commit, preserves all PR commits), Squash and merge (combines all PR commits into one on main — cleanest for trunk-based dev), Rebase and merge (replays PR commits linearly without a merge commit). Most enterprise teams enforce squash-only for a clean, readable main history."},
