...
<div class="cheat-item"><div class="cheat-cmd">git gc</div><div class="cheat-desc">Optimise repo</div></div>
  ...
  challenges:[
    {q:"Type the command that shows a full history of every position HEAD has ever been — including commits that appear to have been deleted.",scenario:"You've been working all day and accidentally reset --hard to the wrong commit. What command reveals your escape route?",hint:"It's Git's safety net — a log of all HEAD movements. Shorthand for 'reference log'.",answer:"Answer: git reflog — The \'reference log\' records every HEAD movement: commits, branch switches, resets, merges, rebases. Entries expire after ~90 days. Always run this first when you think you\'ve lost work.",accept:["git reflog","git reflog ","git reflog show"],feedback:"git reflog is the ultimate safety net. Every commit, branch switch, reset, merge, and rebase is logged here. Entries expire after ~90 days. Always run this first when you think you've lost work."},
    {q:"Type the command to search all tracked files in the repo for the text 'console.log'.",scenario:"Before a production release, you want to find any debug statements left in the codebase.",hint:"Git has a built-in search: 'git grep' followed by your search term.",answer:"Answer: git grep \"console.log\" — Git\'s built-in search is faster than regular grep because it only searches tracked files. Add -i for case-insensitive, -n for line numbers, --count for file summaries.",accept:["git grep \"console.log\"","git grep 'console.log'","git grep console.log"],feedback:"git grep is much faster than regular grep because it only searches Git-tracked files. It's case-sensitive by default. Add -i for case-insensitive. Use --count to show just the file counts."},
