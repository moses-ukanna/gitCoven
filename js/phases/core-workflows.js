...
<span class="hl">git pull</span> --rebase                 <span class="cmt"># fetch + rebase (cleaner)</span>`}},
  ...
  challenges:[
    {q:"Type the command to add a remote named 'origin' pointing to 'https://github.com/user/repo.git'.",scenario:"You have a local repo and just created an empty repo on GitHub. Connect them.",hint:"The command pattern is: git remote add <name> <url>. Fill in the name and URL from the question.",accept:["git remote add origin https://github.com/user/repo.git"],feedback:"git remote add creates an alias (origin) for the remote URL. After this, you can push/pull without typing the full URL every time."},
    {q:"Type the full command to push your 'main' branch to origin for the very first time AND set it as the default upstream.",scenario:"First push ever. You want future 'git push' commands to know where to go without specifying explicitly.",hint:"You need 'git push' with a flag that sets the upstream tracking. The flag is -u.",accept:["git push -u origin main","git push --set-upstream origin main","git push --set-upstream-to origin main"],feedback:"The -u flag (short for --set-upstream) creates a tracking relationship between your local main and origin/main. After this, just git push or git pull works without arguments."},
    {q:"What is the difference between git fetch and git pull? Answer in one sentence.",scenario:"A teammate asks you to explain the difference clearly.",hint:"One downloads only. The other downloads AND does something extra. What extra step does pull add?",accept:["fetch downloads without merging, pull downloads and merges","fetch downloads changes without merging them, pull fetches and merges","fetch gets changes without merging, pull fetches and merges automatically","fetch downloads remote changes but does not merge, pull fetches and then merges","git fetch downloads without merging git pull fetches and merges"],feedback:"Correct. git fetch is the safe operation — it updates your remote-tracking branches (origin/main) but leaves your local branches untouched. git pull = git fetch + git merge (or rebase with --rebase)."}
  ]
}
);