...
<span class="hl">git remote remove</span> gitlab</span>`}}
  ...
  challenges:[
    {q:"Type the safe force push command you should ALWAYS use instead of git push --force.",scenario:"You rebased your feature branch and need to force push it to GitHub. What is the safe version of force push?",hint:"There's a safer version of --force that checks for remote changes first: --force-with-lease.",accept:["git push --force-with-lease","git push --force-with-lease origin","git pushf","git push --force-with-lease origin main"],feedback:"--force-with-lease is the professional's force push. It checks that the remote branch hasn't been updated since you last fetched — if a teammate pushed in the meantime, the command fails and warns you, protecting their work. Plain --force blindly overwrites whatever is on the remote."},
    {q:"Type the command to delete a remote branch called 'feature-old' on origin.",scenario:"A feature branch was merged and you want to clean it up from the remote.",hint:"Use 'git push origin --delete' followed by the branch name.",accept:["git push origin --delete feature-old","git push origin :feature-old","git push --delete origin feature-old"],feedback:"git push origin --delete branch-name sends a delete instruction to the remote. The older syntax git push origin :branch-name (colon prefix) also works. Note: this only deletes the remote branch — your local branch still exists until you run git branch -d feature-old."},
    {q:"Type the command to remove stale remote-tracking branches that no longer exist on the remote server.",scenario:"Your local git branch -r shows 20 remote branches, but most were deleted on GitHub months ago. Clean them up.",hint:"Use 'git fetch' with the --prune flag to clean up deleted remote branches.",accept:["git fetch --prune","git fetch -p","git remote prune origin","git fetch --prune origin"],feedback:"git fetch --prune downloads new remote changes AND removes any local remote-tracking refs (like origin/old-branch) for branches that have been deleted on the server. Set it automatically with: git config --global fetch.prune true"},
    {q:"Type the GitHub CLI command to create a new PUBLIC repository called 'my-app' on GitHub from your current directory and push it immediately.",scenario:"You want to publish a local project to GitHub in a single command without touching the browser.",hint:"Use 'gh repo create' with --public, --source=. and --push flags.",accept:["gh repo create my-app --public --source=. --push","gh repo create my-app --public --source . --push","gh repo create my-app --public --source=. --push ","gh repo create my-app --public --push --source=.","gh repo create my-app --public --push --source ."],feedback:"This single gh CLI command creates the GitHub repo, adds it as origin, and pushes your local commits — all in one step. It's the fastest way to publish a new project. gh repo create without --source creates an empty repo on GitHub only."}
  ]
}
);