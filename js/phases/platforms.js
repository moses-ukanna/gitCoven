...
<span class="cmt"># origin  git@github.com:USERNAME/REPO.git (push)</span></span>`}}
  ...
  challenges:[
    ...
    {q:"Your push was rejected with 'error: failed to push some refs'. What does this mean and what command do you run to fix it safely?",scenario:"A teammate pushed to main while you were working. Your push got rejected because your local branch is behind the remote.",hint:"The remote has commits you don't have yet. Pull with --rebase first, then push again.",accept:["git pull --rebase then git push","git pull --rebase","pull --rebase then push","git pull --rebase origin main","run git pull --rebase then git push"],feedback:"A rejected push means the remote has commits your local branch doesn't have yet. git pull --rebase fetches the remote commits and replays your local commits on top, giving a clean linear history. Then git push succeeds. Never force push on shared branches to 'fix' a rejection."}
  ]
},
{
...
<span class="cmt"># origin  git@github.com:USERNAME/REPO.git (fetch)</span></span>`}},
  ...
  challenges:[
    ...
    {q:"Your push fails with 'Support for password authentication was removed'. What are the two modern authentication methods you can use instead?",scenario:"A new developer on your team gets this error on their first push. What do you tell them?",hint:"The two modern methods: SSH keys (keypair-based) and Personal Access Tokens (generated in settings).",accept:["ssh and personal access token","ssh key and personal access token","personal access token and ssh","pat and ssh","ssh or pat","ssh keys and pats"],feedback:"GitHub removed password authentication in August 2021. The two alternatives are: SSH keys (generate a key pair, add public key to GitHub — recommended for daily use) or Personal Access Tokens (generate in Developer Settings, use as password when prompted — good for scripts and CI)."}
  ]
},
{
...
<span class="hl">git remote remove</span> gitlab</span>`}}
  ...
  challenges:[
    ...
    {q:"Type the GitHub CLI command to create a new PUBLIC repository called 'my-app' on GitHub from your current directory and push it immediately.",scenario:"You want to publish a local project to GitHub in a single command without touching the browser.",hint:"Use 'gh repo create' with --public, --source=. and --push flags.",accept:["gh repo create my-app --public --source=. --push","gh repo create my-app --public --source . --push"],feedback:"This single gh CLI command creates the GitHub repo, adds it as origin, and pushes your local commits — all in one step. It's the fastest way to publish a new project. gh repo create without --source creates an empty repo on GitHub only."}
  ]
}
);