...
<span class="hl">git switch</span> -c recovered HEAD@{1}  <span class="cmt"># or branch from it</span>`}},
  ...
  challenges:[
    {q:"A bad commit was pushed to the shared main branch. What is the safest command to undo it without rewriting history?",scenario:"5 teammates have already pulled this commit. You cannot use reset. What do you use?",hint:"You need a command that creates a NEW commit undoing the changes — it doesn't erase history. Think 'reverse'.",accept:["git revert head","git revert HEAD","git revert HEAD~0"],feedback:"git revert creates a NEW commit that undoes the changes of the target commit. History is preserved — safe for shared branches. Never use git reset --hard on commits others have already pulled."},
    {q:"Type the command to undo the last commit but keep all its changes ready in the staging area.",scenario:"You committed too soon and want to add one more file to the same commit.",hint:"It's 'git reset' with a softness flag and HEAD~1. The flag name describes how gently it treats your files.",accept:["git reset --soft head~1","git reset --soft HEAD~1","git reset --soft head~1 "],feedback:"git reset --soft HEAD~1 moves the branch pointer back by one commit but leaves all the changes staged. You can then stage additional files and commit everything together."},
    {q:"You ran git reset --hard and seem to have lost 3 commits. What command do you run FIRST to find those lost commits?",scenario:"Panic. Your work seems gone. What is your immediate next command?",hint:"There's a command that records EVERY movement of HEAD — even things that seem deleted. Think 'reference log'.",accept:["git reflog","git reflog ","git reflog show"],feedback:"git reflog records every single movement of HEAD — including commits that appear deleted. You'll see HEAD@{N} entries for each past state. Find the hash you need and git reset --hard to it or branch from it."}
  ]
},
{
...
<span class="hl">git stash clear</span>                    <span class="cmt"># delete ALL stashes</span>`}},
  ...
  challenges:[
    {q:"Type the command to save your current work-in-progress to the stash with the label 'login form half done'.",scenario:"You need to urgently switch branches but aren't ready to commit your current changes.",hint:"Use 'git stash push' with the -m flag for a message, just like git commit -m.",accept:["git stash push -m \"login form half done\"","git stash push -m 'login form half done'","git stash save \"login form half done\"","git stash save 'login form half done'"],feedback:"git stash push -m 'message' creates a named stash. Named stashes are much easier to identify later when you have multiple stashes. git stash save is the older deprecated syntax."},
    {q:"Type the command to restore your most recent stash AND remove it from the stash list.",scenario:"You're back on your feature branch and want to continue where you left off.",hint:"There are two restore commands: one keeps the stash, one removes it. You want the one that 'pops' it off the stack.",accept:["git stash pop","git stash pop "],feedback:"git stash pop is the most commonly used stash command. It applies the top stash and removes it from the stack. If you want to keep it in the list (to apply to other branches), use git stash apply instead."},
    {q:"Type the command to see a full diff of what is inside your most recent stash.",scenario:"You have 3 stashes and can't remember what's in the latest one. You want to see the actual changes.",hint:"It's 'git stash show' with a flag that shows the full patch, not just a summary.",accept:["git stash show -p","git stash show --patch","git stash show -p stash@{0}"],feedback:"git stash show -p (patch) shows the full diff of the stash. Without -p, it shows a summary of which files changed. Add stash@{N} to inspect a specific stash."}
  ]
}
);