...
<span class="hl">git config</span> --global alias.undo <span class="str">"reset --soft HEAD~1"</span>`}},
  ...
  challenges:[
    {q:"Type the exact command to set your global Git username to 'Ada Lovelace'.",scenario:"You just installed Git on a new machine. This is the first configuration command you must run.",hint:"The command starts with 'git config' and needs the scope flag for all repos on your account.",accept:["git config --global user.name \"ada lovelace\"","git config --global user.name 'ada lovelace'","git config --global user.name ada lovelace"],feedback:"git config --global user.name sets the author name attached to every future commit on this machine. Use quotes around names with spaces."},
    {q:"Type the command to view all your current Git configuration settings.",scenario:"You want to confirm your name and email are correctly saved before making your first commit.",hint:"It's a git config command with a flag that shows everything — think 'list' all settings.",accept:["git config --list","git config --list ","git config -l"],feedback:"git config --list shows all config values from system, global, and local scopes combined. You can also open ~/.gitconfig directly with any text editor."},
    {q:"Which scope flag do you use to make a config setting apply to ALL repositories for your user account?",scenario:"You want your email to apply everywhere, not just in the current folder.",hint:"Three scopes: --system (everyone), ______ (your user), --local (this repo only).",accept:["--global","-global","global"],feedback:"--global saves the setting to ~/.gitconfig which applies to all repositories owned by your user account. It's the right scope for identity settings like name and email."}
  ]
},
{
...
<span class="cmt"># s = split into smaller chunks</span>`}}
  ...
  challenges:[
    ...
    {q:"What flag do you add to git add to interactively choose which specific chunks of a file to stage?",scenario:"You changed 5 things in app.js but only want to stage 2 of them in this commit.",hint:"It stands for 'patch' — a single-letter flag that lets you review each change hunk individually.",accept:["-p","--patch","-p flag","--patch flag"],feedback:"git add -p (or --patch) opens an interactive mode that shows each change 'hunk' and lets you choose y (yes), n (no), s (split), or q (quit). Extremely useful for crafting clean, atomic commits."}
  ]
},
{
...
<span class="hl">git diff</span> --staged          <span class="cmt"># staged changes</span>`}},
  ...
  challenges:[
    {q:"Type the command to initialise a new Git repository in the current directory.",scenario:"You have a project folder with code in it. You want Git to start tracking it.",hint:"Two words. The second word is short for 'initialise'.",accept:["git init","git init ."],feedback:"git init creates the .git directory inside the current folder. From this point on, Git tracks all changes in this directory. It does not affect your existing files."},
    {q:"Type the command to stage ALL changes in the current directory for the next commit.",scenario:"You've edited 4 files and want to include all of them in one commit.",hint:"'git add' followed by a symbol that means 'everything here' — just one character.",accept:["git add .","git add -a","git add -a ","git add --all","git add -a","git add -A"],feedback:"git add . stages all new and modified files in the current directory and subdirectories. git add -A also includes deletions across the whole working tree."},
    {q:"Type the command to commit your staged changes with the message 'Add homepage'.",scenario:"You have staged files ready. Write the exact commit command.",hint:"Use the -m flag followed by your message in quotes.",accept:["git commit -m \"add homepage\"","git commit -m 'add homepage'","git commit -m add homepage"],feedback:"git commit -m allows you to provide the commit message inline. Without -m, Git opens your configured editor so you can write a longer multi-line message."},
    {q:"Type the command to see a compact one-line-per-commit view of the entire history.",scenario:"You want a quick overview of all the commits in this repo without the full metadata.",hint:"It's 'git log' with a flag that condenses output to one line per commit.",accept:["git log --oneline","git log --oneline "],feedback:"git log --oneline shows each commit as: HASH Message. Combine with --graph --all for a visual branch diagram."}
  ]
}
);