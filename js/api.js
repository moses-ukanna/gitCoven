...
function switchTab(tab) {
  currentTab = tab;
  var forms = ['login','register','magic','forgot'];
  for (var i = 0; i < forms.length; i++) {
    var el = document.getElementById('form-' + forms[i]);
    if (el) el.style.display = forms[i] === tab ? 'block' : 'none';
  }
  // Hide main tabs and OAuth section when on forgot view
  var tabs = document.querySelectorAll('.auth-tab');
  var oauthSection = document.querySelector('.auth-body > div:first-child'); // OAuth buttons
  var divider = document.querySelector('.auth-body > div:nth-child(3)');     // OR divider
  if (tab === 'forgot') {
    for (var i = 0; i < tabs.length; i++) tabs[i].style.display = 'none';
    if (oauthSection) oauthSection.style.display = 'none';
    if (divider) divider.style.display = 'none';
  } else {
    for (var i = 0; i < tabs.length; i++) { tabs[i].style.display = ''; tabs[i].classList.remove('active'); }
...
function showPasswordResetForm() {
  ...
  // Hide all auth forms, tabs, OAuth, and divider
  ...
  if (authBody) {
    var oauthSection = authBody.querySelector('div:first-child');
    var divider = authBody.children[2];
    if (oauthSection) oauthSection.style.display = 'none';
    if (divider) divider.style.display = 'none';
  }
  // Show reset form
